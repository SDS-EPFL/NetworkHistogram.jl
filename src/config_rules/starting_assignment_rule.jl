...
function initialise_node_labels(A, h; starting_assignment_rule::OrderedStart)
    group_size = GroupSize(size(A,1), h)
    inverse_rle(1:length(group_size), group_size)
    return node_labels, group_size
end

function initialise_node_labels(A, h; starting_assignment_rule::RandomStart)
    node_labels, group_size = initialise_node_labels(A, h; starting_assignment_rule = OrderedStart())
