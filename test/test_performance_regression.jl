...
function create_test_sbm_categorical(n_groups::Int, n_nodes::Int, n_categories::Int; seed=42)
    Random.seed!(seed)
...
@testset "Performance Regression Tests" begin
    ...
    @testset "Bernoulli Networks" begin
        @testset "Small network (n=50, k=2)" begin
            ...
            @info "Bernoulli (n=50, k=2) - Single swap" median=median(b_swap.times)/1e6 mean=mean(b_swap.times)/1e6
        end
        ...
        @testset "Medium network (n=200, k=3)" begin
            ...
            @info "Bernoulli (n=200, k=3) - Single swap" median=median(b_swap.times)/1e6 mean=mean(b_swap.times)/1e6
        end
        ...
        @testset "Large network (n=500, k=5)" begin
            ...
            @info "Bernoulli (n=500, k=5) - Single swap" median=median(b_swap.times)/1e6 mean=mean(b_swap.times)/1e6
        end
    end
    ...
    @testset "Categorical Networks" begin
        @testset "Small network (n=50, k=2, m=3)" begin
            ...
            @info "Categorical (n=50, k=2, m=3) - Single swap" median=median(b_swap.times)/1e6 mean=mean(b_swap.times)/1e6
        end
        ...
        @testset "Medium network (n=200, k=3, m=4)" begin
            ...
            @info "Categorical (n=200, k=3, m=4) - Single swap" median=median(b_swap.times)/1e6 mean=mean(b_swap.times)/1e6
        end
        ...
        @testset "Large network (n=500, k=5, m=5)" begin
            ...
            @info "Categorical (n=500, k=5, m=5) - Single swap" median=median(b_swap.times)/1e6 mean=mean(b_swap.times)/1e6
        end
    end
    ...
    @testset "Full Optimization Workflow" begin
        @testset "Bernoulli - Short optimization (n=100, k=3)" begin
            ...
            @info "Bernoulli full optimization (n=100, 1k iters)" median=median(b_optimize.times)/1e6 mean=mean(b_optimize.times)/1e6
        end
        ...
        @testset "Categorical - Short optimization (n=100, k=3, m=3)" begin
            ...
            @info "Categorical full optimization (n=100, 1k iters)" median=median(b_optimize.times)/1e6 mean=mean(b_optimize.times)/1e6
        end
    end
    ...
    @testset "Component Benchmarks" begin
        @testset "Assignment creation (n=200, k=3)" begin
            ...
            @info "Assignment creation (n=200)" median=median(b_assignment.times)/1e6 mean=mean(b_assignment.times)/1e6
        end
        ...
        @testset "EdgeList creation (n=200)" begin
            ...
            @info "EdgeList creation (n=200)" median=median(b_edgelist.times)/1e6 mean=mean(b_edgelist.times)/1e6
        end
        ...
        @testset "Loglikelihood computation (n=200, k=3)" begin
            ...
            @info "Loglikelihood computation (n=200)" median=median(b_ll.times)/1e3 mean=mean(b_ll.times)/1e3
        end
        ...
        @testset "Get edges in groups (n=200, k=3)" begin
            ...
            @info "Get edges in groups (n=200)" median=median(b_get_edges.times)/1e3 mean=mean(b_get_edges.times)/1e3
        end
    end
end
