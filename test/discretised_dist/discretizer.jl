...
@testset "discretizer" begin
    using StaticArrays
    reg_disc = NetworkHistogram.RegularDiscretizer(10, 0.0, 1.0, MVector{10}(1:10), 1 / 10)
    ...
    @test all(NetworkHistogram.encode(reg_disc, 0.001:0.001:1.0) .== NetworkHistogram.encode(hybrid_disc, 0.001:0.001:1.0))
    @test all(NetworkHistogram.decode(hybrid_disc, 1:10) .== NetworkHistogram.decode(reg_disc, 1:10))
end
