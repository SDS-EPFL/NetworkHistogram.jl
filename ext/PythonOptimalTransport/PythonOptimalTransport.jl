...
function get_perm_alignment(
        src::AbstractMatrix{<:Real}, target::AbstractMatrix{<:Real}; kwargs...)
...
function get_perm_alignment(src::AbstractMatrix{T1}, target::AbstractMatrix{T2};
        kwargs...) where {T1 <: AbstractVector, T2 <: AbstractVector}
    C1 = jl_to_np(target)
    C2 = jl_to_np(src)
    ...
    return plan_to_permutation(plan), plan
end
